...
for i in ['lobatto']:
    ...
    # print(list(time_solution))
    # print(list(problem.solution.state[0][2]))
    plt.subplot(3, 1, 1)
    plt.plot(time_solution, position_solution, marker="x",label = i)
    plt.title('position')

    plt.subplot(3, 1, 2)
    plt.plot(time_solution, angle_solution, marker="x",label = i)
    plt.title('angle')
    index = [i for i in range(len(control_solution)) if np.round(control_solution[i],3) == 0.0]
    control_solution = np.delete(control_solution,index)
    time_solution = np.delete(time_solution,index)
    plt.subplot(3, 1, 3)
    plt.plot(time_solution, control_solution, marker="x",label = i)
    plt.title('force')
plt.legend()
plt.show()
